...
int main(int argc, char *argv[]) {
    ...
    // Use POSIX_FADV_SEQUENTIAL to hint sequential access
    if (posix_fadvise(fd, 0, st.st_size, POSIX_FADV_SEQUENTIAL) < 0) {
        perror("Failed to set fadvise");
        // Continue execution even if fadvise fails, as it's a hint
    }
